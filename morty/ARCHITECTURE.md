...
### Audio (no conversion — confirmed from AssemblyAI Twilio docs)

Twilio G.711 μ-law is **byte-compatible** with AssemblyAI `audio/pcmu`. Set both input and output encoding to `audio/pcmu` and forward base64 payloads directly — zero transcoding.

| Direction | Format | Sample Rate | Action |
|-----------|--------|-------------|--------|
| Twilio → bridge | μ-law (PCMU) base64 | 8 kHz | Forward as-is to AssemblyAI |
| bridge → AssemblyAI | `input.audio` JSON event | 8 kHz | `{ type: "input.audio", audio: msg.media.payload }` |
| AssemblyAI → bridge | `reply.audio` base64 in `data` field | 8 kHz | Forward to Twilio |
| bridge → Twilio | μ-law (PCMU) base64 | 8 kHz | `{ event: "media", streamSid, media: { payload: event.data } }` |

### Correct session.update encoding (from live docs)
...
```json
{
  ...
  "session": {
    ...
    "output": { "type": "audio", "voice": "emma", "format": { "encoding": "audio/pcmu" } }
  }
}
```
...
On `input.speech.started` event → send Twilio `clear` to stop audio playback immediately:
```json
